...
function Metronome() {
  ...
  const updateBeatsPerMeasure = (value) => {
    const nextValue = clamp(value, 2, 8);
  ...
  useEffect(() => {
    ...
    return clearTimer;
  }, [clearTimer, intervalMs, playing, tick]);
      ...
      <section className="metronome-card" aria-label="Browser metronome">
        ...
        <div className="beat-row" aria-label="Beat indicator">
          {Array.from({ length: beatsPerMeasure }, (_, index) => {
            ...
            const isActive = playing && currentBeat === beatNumber;
        ...
        <button
          className={`primary-action${playing ? " is-playing" : ""}`}
          type="button"
